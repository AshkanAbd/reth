    ...
    fn insert_block_inner(
        ...
        let (state_root_handle, state_root_task_config, state_root_sender, state_hook) =
            if is_descendant_of_persisting_blocks && self.config.use_state_root_task() {
                ...
                info!(target: "engine::tree", block=?block_num_hash, elapsed = ?trie_input_elapsed, "Computed trie input for state root task");

                let new_from_input_start = Instant::now();
                let state_root_config =
                    StateRootConfig::new_from_input(consistent_view, trie_input);
                let new_from_input_elapsed = new_from_input_start.elapsed();
                info!(target: "engine::tree", block=?block_num_hash, elapsed = ?new_from_input_elapsed, "Created state root config from input");
